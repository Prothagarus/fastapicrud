...
def update_todo(payload:List[schemas.ToDo],session: Session = Depends(get_session)):

    for todo in payload:
            
        #todo = session.query(models.ToDo).get(id)     # get given id
                itemexist = session.query(models.ToDo).filter(models.ToDo.id == todo.id ).first()    # get given id
